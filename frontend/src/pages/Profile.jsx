...
const Profile = () => {
  ...
  const [phoneSuccess, setPhoneSuccess] = useState('')
  const { isDark } = useTheme()
  const { location: savedGlobalLocation, openLocationModal } = useDsLocation()
  const navigate = useNavigate()
              ...
              <div className="px-6 py-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <p className={`mt-1 text-base font-medium flex items-center gap-2 ${isDark ? 'text-white' : 'text-gray-900'}`}>
                      {savedGlobalLocation
                        ? `📍 ${savedGlobalLocation.city}, ${savedGlobalLocation.state}`
                        : 'Not set'}
                      <button
                        onClick={openLocationModal}
                        className="text-xs font-semibold text-yellow-600 dark:text-yellow-400 underline"
                      >
                        Change
                      </button>
                    </p>
